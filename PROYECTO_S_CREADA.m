...
%si la conexión fue exitosa, cliente será diferente
if (clientID>-1)
    disp('Conectado correctamente');
    %utilizamos un handler para coger cada joint que se encuentra escena de
    %coppelia.
...
axe(1)=axes('parent',fig(1),'units','pixels','position',[120,300,400,200],'xlim',[0 120],'ylim',[-5 max(ext)],'xgrid','on','ygrid','on');
    %se crea la gráfica1 (axe1) donde se incorporará los siguientes valores
        ...
        lin(1)=line('parent',axe(1),'xdata',[],'ydata',[],'Color','r','LineWidth',0.2);
    ...
    axe(2)=axes('parent',fig(1),'units','pixels','position',[120,50,400,200],'xlim',[0 120],'ylim',[-5 max(ext)],'xgrid','on','ygrid','on');
        ...
        lin(2)=line('parent',axe(2),'xdata',[],'ydata',[],'Color','r','LineWidth',0.2);
    ...
    %se crea la gráfica1 (axe1) donde se incorporará los siguientes valores
        ...
        lin(3)=line('parent',axe(3),'xdata',[],'ydata',[],'Color','r','LineWidth',0.2);
    movegui(fig(1),'center');%se centra la figura al centro
%CREACIÓN DE FIGURA

talla=size(tiempo);%tamaño de la muestra
disp(talla)%lote es el tamaño el cual recorrerá el bucle, es decir de cada iteración, se cogerán del vector el lote y de ahí se obtendrá un punto que se incorporaá en la gráfica
x1=[0];%vectores sup /flex
x2=[0];%vectores sup /ext
tt=[0];%vectores sup /tiempo
S=[0];%vector sup /salida
tpi=MAX/100;
pos_munyeca=0;
pos_antebrazo=0;
apert=0;
...
extens_munyeca=0;
    ...
    for t=1:1500:(talla(1)-1500)%bucle que recorrerá de 1500 en 1500 puntos con el fin de ir creando ese movimiento continuo en la gráfica
